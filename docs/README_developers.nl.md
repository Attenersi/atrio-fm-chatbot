...
Reproduceerbare installs: **[uv](https://docs.astral.sh/uv/)** + [`backend/pyproject.toml`](../backend/pyproject.toml) en [`backend/uv.lock`](../backend/uv.lock). Lokaal: `cd backend && uv sync --group dev` en bv. `uv run pytest tests -q`. CI gebruikt `uv sync --frozen --group dev`.

[`backend/requirements.txt`](../backend/requirements.txt) is een **export** voor Docker/pip; opnieuw genereren na wijzigingen: `uv lock` en `uv export --format requirements-txt --no-hashes --no-dev -o requirements.txt` (zie Engelstalige [README_developers.md](README_developers.md)).

## Architectuur en requestflow

Zie **[`docs/architecture.md`](architecture.md)** (EN) voor de canonieke Mermaid-diagrammen. Voeg die uitleg niet opnieuw toe in andere documenten; link hierheen.
...
- `backend/app/database.py` - schema + persistence (zie ook [`docs/schema.md`](schema.md))
...
GitHub Actions: [`ci.md`](ci.md) (EN).

## Security / RAG

Prompt injection en guardrails (EN): [`prompt_injection_and_guardrails.md`](prompt_injection_and_guardrails.md).

...
**Untrusted retrieval:** Chroma chunks are not trusted as policy. The backend marks retrieved text as reference-only in the system prompt ([`backend/app/rag.py`](../backend/app/rag.py)) and applies deterministic post-LLM rules ([`backend/app/classifier.py`](../backend/app/classifier.py), [`backend/app/main.py`](../backend/app/main.py)). Threat model and operations: [`prompt_injection_and_guardrails.md`](prompt_injection_and_guardrails.md).
...
- **Why SQLite / when Postgres**: [`docs/adr/0001-sqlite-operational-database.md`](adr/0001-sqlite-operational-database.md).
- **HTTP request correlation**: [`docs/observability.md`](observability.md) — `X-Request-ID` on requests/responses.

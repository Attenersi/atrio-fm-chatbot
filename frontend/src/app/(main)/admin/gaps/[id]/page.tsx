...
export default function ResolveGapPage() {
  ...
  async function resolveGap() {
    ...
    if (!DOC_NAME_REGEX.test(normalizedDocName)) {
      setStatus("Document name must use only letters/numbers/_/.- and end with .md or .txt");
      return;
    }
    ...
    try {
      ...
      const reindexMsg =
        res.chunks_indexed !== null ? ` Reindexed ${res.chunks_indexed} chunks.` : "";
    ...
    <section className="page-shell">
      ...
      <div className="card panel-grid">
        ...
        {!useCustomDocName || !customDocAllowed ? (
          <select id="doc-name" value={docName} onChange={(e) => setDocName(e.target.value)}>
            {docs.map((d) => (
        ...
        <select id="mode" value={mode} onChange={(e) => setMode(e.target.value as "append" | "overwrite")}>
          <option value="append">append (add below existing text)</option>
          <option value="overwrite">update existing document (edit full text)</option>
        </select>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={resolveGap} disabled={busy || !gap || loadingDocContent}>
            Resolve & Save to KB
          </button>
          <button
